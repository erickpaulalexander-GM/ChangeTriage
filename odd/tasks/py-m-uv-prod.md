...
- **2026-09-24 camino B (offline total):** el banco notificó por descargas automáticas de PyPI. Decisión del usuario: eliminar TODO download del runtime. run.bat ahora corre 100% offline usando paquetes ya instalados en `py` (sin `uv`, sin `--with`); Install-Dependencias.py instala openpyxl/tzdata/pytest una vez desde `wheels\` local (`--no-index --find-links`); `wheels/` agregado a .gitignore; Comandos-Prod.txt y README actualizados. Verificado: 12 pytest + 221 smoke + pipeline 1209 filas SIN internet (uv ni siquiera instalado como requisito).
